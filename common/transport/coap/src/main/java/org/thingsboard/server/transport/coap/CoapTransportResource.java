...
public class CoapTransportResource extends AbstractCoapTransportResource {
    ...
    private void handleToDeviceRpcResponse(TbCoapClientState clientState, CoapExchange exchange, Request request) throws AdaptorException {
        ...
        ToDeviceRpcResponseMsg rpcResponseMsg = clientState.getAdaptor().convertToDeviceRpcResponse(sessionId, request, clientState.getConfiguration().getRpcResponseMsgDescriptor());
        transportService.process(session, rpcResponseMsg, new CoapOkCallback(exchange, CoAP.ResponseCode.CREATED, CoAP.ResponseCode.INTERNAL_SERVER_ERROR));
        clients.onToDeviceRpcResponse(rpcResponseMsg.getRequestId());
    }
