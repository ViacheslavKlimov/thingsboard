...
public class DefaultCoapClientContext implements CoapClientContext {
    ...
    private final Set<Integer> rpcRequestsAwaitingResponse = new HashSet<>();
    ...
    public class CoapSessionListener implements SessionMsgListener {
        ...
        public void onToDeviceRpcRequest(UUID sessionId, TransportProtos.ToDeviceRpcRequestMsg msg) {
            ...
            try {
                ...
                if (msg.getOneway()) {
                    if (conRequest) {
                        scheduleRpcRequestTimeout(msg, requestId, ApiUsageRecordKey.FAILED_ONE_WAY_RPC_REQUEST_COUNT);
                    }
                } else {
                ...
                response.addMessageObserver(new AbstractCoapTransportResource.CoapRequestFailureMessageObserver(() -> {
                    ...
                    if (rpcRequestMsg != null) {
                        transportContext.getApiUsageReportClient().report(TransportService.getTenantId(state.getSession()),
                                TransportService.getCustomerId(state.getSession()), msg.getOneway() ?
        ...
        private void scheduleRpcRequestTimeout(TransportProtos.ToDeviceRpcRequestMsg rpcRequest, int msgId, ApiUsageRecordKey key) {
            transportContext.getScheduler().schedule(() -> {
                ...
                if (notAcknowledged || notReplied) {
                    transportContext.getApiUsageReportClient().report(TransportService.getTenantId(state.getSession()),
                            TransportService.getCustomerId(state.getSession()), key);
                }
    ...
    @Override
    public void onToDeviceRpcResponse(int requestId) {
        rpcRequestsAwaitingResponse.remove(requestId);
    }
